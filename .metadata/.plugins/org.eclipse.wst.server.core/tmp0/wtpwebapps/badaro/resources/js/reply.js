...
$(document).ready(function(){
	var p_turnValue=$("#p_turn").val();	// 게시판 번호(6)
	var replyUL=$(".chat")
	
	console.log(p_turnValue);
	...
	function showList(page){ // showList함수 선언 시작
							// 6 : 6         1 :  1
		replyService.getList({p_turn:p_turnValue,page:page||1},function(replyCnt,list){// getList 함수 호출 시작
...
var replyService = (function(){
	...
	function getList(param,callback,error){// getList함수 시작(댓글 목록리스트를 처리하기 위한 함수 선언)
		var p_turn = param.p_turn;	// 게시판번호
		var page = param.page || 1;	// 페이지번호
		//                 /pages/6/1.json
		$.getJSON("/replies/pages/"+p_turn+"/"+page+".json",	// getJSON 시작
				function(data){
					if(callback){
